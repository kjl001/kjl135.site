...
async function createPerf(data) {
	const result = await pool.query(`IF EXISTS (SELECT * FROM performance WHERE id = ?) THEN INSERT INTO performance VALUES (?, ?, ?, ?); END IF;`, [data['id'], data['id'], data['loadStart'], data['loadEnd'], data['totalLoad']]);
	return result;
}
